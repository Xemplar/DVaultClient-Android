...
public final class Sha256 {
    public static final int BLOCK_LEN = 64;  // In bytes
    ...
    public static byte[] getHmac(byte[] key, byte[] msg) {
        ...
        key = Arrays.copyOf(key.length <= BLOCK_LEN ? key : getHash(key), BLOCK_LEN);
        ...
        byte[] innerHash = getHash(msg, state, key.length);
        ...
        compress(state, key, key.length);
        return getHash(innerHash, state, key.length);
    }
